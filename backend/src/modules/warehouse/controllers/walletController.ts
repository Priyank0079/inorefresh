...
/**
 * Get Warehouse's wallet statistics
 */
export const getWalletStats = asyncHandler(async (req: Request, res: Response) => {
    ...
    const earningsData = await WalletTransaction.aggregate([
        { $match: { WarehouseId: new mongoose.Types.ObjectId(WarehouseId), type: 'Credit', description: { $regex: /Order/i } } },
    ...
    const withdrawnData = await WithdrawRequest.aggregate([
        { $match: { WarehouseId: new mongoose.Types.ObjectId(WarehouseId), status: 'Completed' } },
        { $group: { _id: null, total: { $sum: '$amount' } } }
    ]);

    // Pending settlements (OrderItem total - net earnings if not yet settled in wallet)
    // For simplicity, we'll fetch items assigned to this Warehouse that are not 'Cancelled' but not yet reflected in wallet balance if applicable
