...
export const getOrderById = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    // Get only this Warehouse's order items
    const orderItems = WarehouseItems;
...
/**
 * Update order status (Warehouse can update: Accepted, On the way, Delivered, Cancelled)
 */
export const updateOrderStatus = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    // Validate allowed status updates for Warehouse
    const allowedStatuses = ['Accepted', 'On the way', 'Delivered', 'Cancelled', 'Rejected'];
    if (!allowedStatuses.includes(status)) {
      return res.status(400).json({
        success: false,
        message: `Invalid status. Warehouse can only update to: ${allowedStatuses.join(', ')}`,
      });
    }
