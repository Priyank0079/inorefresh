...
export const getSellerWalletBalance = async () => {
    try {
        const response = await api.get('/warehouse/wallet-new/balance');
...
export const getSellerWalletTransactions = async (page: number = 1, limit: number = 20) => {
    try {
        const response = await api.get('/warehouse/wallet-new/transactions', {
            params: { page, limit },
        });
        return response.data;
    } catch (error: any) {
...
export const requestSellerWithdrawal = async (amount: number, paymentMethod: 'Bank Transfer' | 'UPI') => {
    try {
        const response = await api.post('/warehouse/wallet-new/withdraw', {
            amount,
            paymentMethod,
        });
        return response.data;
...
export const getSellerWithdrawals = async (status?: string) => {
    try {
        const response = await api.get('/warehouse/wallet-new/withdrawals', {
...
export const getSellerCommissions = async () => {
    try {
        const response = await api.get('/warehouse/wallet-new/commissions');
        return response.data;
    } catch (error: any) {
