...
export default function RiderInspectionControl() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-24">
      ...
      <div className="bg-gradient-to-r from-red-600 to-rose-600 shadow-sm border-b">
        <div className="px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <button onClick={() => navigate('/delivery/route/today')} className="text-white p-1 hover:bg-white/20 rounded-full">
                        ...
                        <div className="space-y-2 pt-2 border-t border-gray-100">
                          {!otpSentFor[ret._id] ? (
                            <button
                              onClick={async () => {
                                try {
                                  setSendingOtpFor(ret._id);
                                  await api.post(`/returns/workflow/rider/send-otp/${ret._id}`);
                                  setOtpSentFor(prev => ({ ...prev, [ret._id]: true }));
                                  toast.success("OTP sent to warehouse");
                                } catch (err: any) { toast.error(err.response?.data?.message || "Failed to send OTP"); }
                                finally { setSendingOtpFor(null); }
                              }}
                              disabled={sendingOtpFor === ret._id}
                              className="w-full py-2.5 bg-blue-600 text-white font-bold rounded-lg text-xs disabled:opacity-60"
                            >
                              {sendingOtpFor === ret._id ? "Sending…" : "Get OTP from Warehouse"}
                            </button>
                          ) : (
                            <>
                              <p className="text-xs text-blue-600 text-center">OTP sent to warehouse — enter below</p>
                              <div className="flex gap-2">
                                <input
                                  value={otpValues[ret._id] || ''}
                                  onChange={(e) => setOtpValues(prev => ({ ...prev, [ret._id]: e.target.value }))}
                                  placeholder="OTP"
                                  inputMode="numeric"
                                  maxLength={4}
                                  className="flex-1 px-3 py-2 rounded-lg border border-gray-300 text-sm text-center font-bold focus:outline-none focus:ring-1 focus:ring-green-500"
                                />
                                <button
                                  onClick={async () => {
                                    const code = otpValues[ret._id]?.trim();
                                    if (!code) { toast.error("Enter OTP"); return; }
                                    try {
                                      setVerifyingOtpFor(ret._id);
                                      const res = await api.post(`/returns/workflow/rider/verify-otp/${ret._id}`, { otp: code });
                                      if (res.data.success) { toast.success(res.data.message || "Return confirmed & refunded!"); fetchOrder(); }
                                    } catch (err: any) { toast.error(err.response?.data?.message || "Invalid OTP"); }
                                    finally { setVerifyingOtpFor(null); }
                                  }}
                                  disabled={verifyingOtpFor === ret._id}
                                  className="px-5 py-2 bg-green-600 text-white font-bold rounded-lg text-xs disabled:opacity-60"
                                >
                                  {verifyingOtpFor === ret._id ? "…" : "Confirm"}
                                </button>
                              </div>
                              <button
                                onClick={async () => {
                                  try {
                                    setSendingOtpFor(ret._id);
                                    await api.post(`/returns/workflow/rider/send-otp/${ret._id}`);
                                    toast.success("OTP resent to warehouse");
                                  } catch (err: any) { toast.error(err.response?.data?.message || "Failed"); }
                                  finally { setSendingOtpFor(null); }
                                }}
                                disabled={sendingOtpFor === ret._id}
                                className="w-full text-blue-600 text-[11px] font-medium py-1"
                              >Resend OTP</button>
                            </>
                          )}
                        </div>
      ...
      <div className="p-4">
        <button
          onClick={() => navigate('/delivery/route/today')}
          className="w-full py-3 rounded-xl font-bold text-sm bg-green-600 text-white active:bg-green-700">
          Back to Route Stops
        </button>
      </div>
