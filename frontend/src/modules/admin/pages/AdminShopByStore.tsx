...
export default function AdminShopByStore() {
  ...
  const [showStoreImageCamera, setShowStoreImageCamera] = useState(false);
  ...
  const processStoreImageFile = async (file: File) => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    await processStoreImageFile(file);
  };

  const handleStoreImageCameraCapture = (file: File) => {
    void processStoreImageFile(file);
  };
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-neutral-200 overflow-hidden">
          ...
          <div className="p-4 sm:p-6 space-y-4">
            ...
            <div>
              ...
              <button
                type="button"
                onClick={() => setShowStoreImageCamera(true)}
                disabled={uploading}
      ...
      {showStoreImageCamera && (
        <CameraCapture
          onCapture={handleStoreImageCameraCapture}
          onClose={() => setShowStoreImageCamera(false)}
        />
      )}
    </div>
  );
