...
export default function WarehouseOrderDetail() {
  ...
  const handleExportPDF = async () => {
    ...
    // Calculate totals — compute from warehouse items only so numbers always add up.
    // orderDetail.subtotal / tax / grandTotal are already warehouse-scoped from the backend,
    // but we recompute from items as a safety net in case order.total leaked through.
    const totalSubtotal = orderDetail.items.reduce((sum, item) => sum + item.subtotal, 0);
    const totalTax = orderDetail.tax;  // proportional share from backend
    const grandTotal = Math.round((totalSubtotal + totalTax) * 100) / 100;
  ...
  const handlePrint = () => {
    ...
    // Calculate totals — compute from warehouse items only
    ...
    <div className="min-h-screen bg-neutral-50 pb-8">
      ...
      <div id="warehouse-print-invoice" className="bg-white rounded-lg shadow-sm border border-neutral-200 overflow-hidden">
        ...
        <div className="bg-white px-4 sm:px-6 py-6">
          ...
          {/* Totals Section — computed from items so numbers always add up */}
          {(() => {
            const itemsSubtotal = orderDetail.items.reduce((s, i) => s + i.subtotal, 0);
            const displayTax = orderDetail.tax; // proportional warehouse tax from backend
            const displayGrandTotal = Math.round((itemsSubtotal + displayTax) * 100) / 100;
            return (
              <div className="border-t border-neutral-200 pt-4">
                <div className="flex flex-col items-end gap-1 text-sm">
                  <div className="flex gap-8">
                    <span className="text-neutral-500">Subtotal:</span>
                    <span className="font-medium text-neutral-900 w-28 text-right">₹{itemsSubtotal.toFixed(2)}</span>
                  </div>
                  {displayTax > 0 && (
                    <div className="flex gap-8">
                      <span className="text-neutral-500">Tax:</span>
                      <span className="font-medium text-neutral-900 w-28 text-right">₹{displayTax.toFixed(2)}</span>
                    </div>
                  )}
                  <div className="flex gap-8 border-t border-neutral-200 pt-2 mt-1">
                    <span className="font-bold text-neutral-800">Grand Total:</span>
                    <span className="font-bold text-teal-700 text-base w-28 text-right">₹{displayGrandTotal.toFixed(2)}</span>
                  </div>
                </div>
              </div>
            );
          ...
          <div className="border-t border-dashed border-neutral-300 pt-4 mt-4">
