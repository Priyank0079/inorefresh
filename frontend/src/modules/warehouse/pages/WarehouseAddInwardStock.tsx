...
export default function WarehouseAddInwardStock() {
    ...
    return (
        <div className="flex flex-col h-full bg-neutral-50 min-h-screen">
            {/* Page Header */}
            <div className="bg-[#12b2a2] text-white p-4 sm:p-6 rounded-lg shadow-sm mb-4 sm:mb-6 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 sm:gap-4 transition-all mx-4 mt-4">
                <div className="flex-1">
                    <h1 className="text-xl sm:text-2xl font-bold">{id ? 'Edit' : 'Add New'} Requirement</h1>
                    <p className="text-teal-50 text-xs sm:text-sm mt-1">Create a new fish requirement for ports</p>
                </div>
                <div className="flex items-center gap-1 sm:gap-2 text-xs sm:text-sm flex-wrap">
                    <Link to="/warehouse" className="text-teal-50 hover:text-white font-medium transition-colors whitespace-nowrap">Home</Link>
                    <span className="text-teal-200">/</span>
                    <Link to="/warehouse/inward-stock/list" className="text-teal-50 hover:text-white font-medium transition-colors whitespace-nowrap hidden sm:inline">Requirements</Link>
                    <span className="text-teal-200 hidden sm:inline">/</span>
                    <span className="text-white font-medium">{id ? 'Edit' : 'Add'}</span>
                </div>
            </div>

            <div className="max-w-full sm:max-w-3xl md:max-w-5xl lg:max-w-6xl mx-auto w-full px-3 sm:px-4 mb-6 sm:mb-8">
                <div className="bg-white rounded-lg sm:rounded-xl shadow-lg sm:shadow-xl border border-neutral-200 overflow-hidden">
                    <form onSubmit={handleSubmit} className="p-0">
                        {/* Table-like Header — hidden on mobile, visible md+ */}
                        <div className="hidden md:grid bg-slate-50 border-b border-neutral-200 px-6 sm:px-8 py-3 sm:py-4 grid-cols-4 lg:grid-cols-7 gap-3 sm:gap-4 text-[9px] sm:text-[10px] font-black text-slate-500 uppercase tracking-widest">
                        ...
                        {/* Form Inputs — Responsive grid with proper mobile spacing */}
                        <div className="p-4 sm:p-6 md:p-8 space-y-6 sm:space-y-8">
                            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3 sm:gap-4 items-start">
                                {/* Requirement ID */}
                            ...
                            {/* Status and Action */}
                            <div className="pt-6 sm:pt-8 border-t border-slate-100 space-y-4 sm:space-y-6">
                                {/* Status Selection */}
                                <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
                                    <span className="text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-widest flex-shrink-0">Status:</span>
                                ...
                                {/* Messages */}
                                {submitMessage && (
                                    <div className={`w-full px-3 sm:px-4 py-2 sm:py-2.5 rounded-lg text-xs sm:text-sm font-medium text-center ${
                                ...
                                <div className="flex flex-col-reverse sm:flex-row gap-3 sm:gap-4 pt-2">
                                    <button
                                        ...
                                        className="w-full sm:w-auto px-4 sm:px-8 py-2.5 sm:py-3 rounded-lg sm:rounded-xl font-bold text-sm text-slate-600 hover:text-slate-800 hover:bg-slate-100 transition-all"
                                    >
                                        Cancel
                                    </button>
                                    <button
                                        type="submit"
                                        disabled={loading}
                                        className="w-full sm:w-auto bg-[#12b2a2] hover:bg-[#0f9689] text-white px-4 sm:px-12 py-2.5 sm:py-3.5 rounded-lg sm:rounded-xl font-black text-xs sm:text-sm uppercase tracking-widest transition-all shadow-lg sm:shadow-xl shadow-teal-500/20 disabled:opacity-50"
                                    >
