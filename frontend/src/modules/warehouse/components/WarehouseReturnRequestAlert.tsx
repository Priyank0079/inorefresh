...
import { ReturnRequestAlert } from '../hooks/useWarehouseSocket';

interface WarehouseReturnRequestAlertProps {
  alert: ReturnRequestAlert | null;
  onClose: () => void;
}

/**
 * Full-screen popup with looping sound shown the moment a customer submits a
 * return that needs the warehouse's approval. Drives the manager straight to
 * the return inbox.
 */
const WarehouseReturnRequestAlert: React.FC<WarehouseReturnRequestAlertProps> = ({ alert, onClose }) => {
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-300">
        ...
        <div className="px-6 py-4 flex items-center justify-between bg-amber-600 text-white">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold">New Return Request</h2>
        ...
        <div className="p-6 text-center">
          <p className="text-neutral-700 mb-2">
            <span className="font-semibold">{alert.customerName}</span> requested a
            return for <span className="font-semibold">Order #{alert.orderNumber}</span>.
          </p>
          <p className="text-neutral-500 mb-6">
            {alert.itemCount} item{alert.itemCount === 1 ? '' : 's'} awaiting your approval.
          </p>
...
export default WarehouseReturnRequestAlert;
